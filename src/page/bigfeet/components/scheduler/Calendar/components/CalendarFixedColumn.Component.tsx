...
const CalendarFixedColumn: FC<CalendarFixedColumnProp> = ({ timeArr }) => {
	...
	return (
		<>
			...
			{timeArr.map((time, index) => getTimeDiv(time, index))}
			<div
				style={{ gridRowStart: timeArr.length + 2 }}
				className="col-start-[1] border-slate-300 border-r whitespace-nowrap p-1.5 text-right text-black uppercase sticky left-0 z-[5] bg-white font-medium">
				{t('Total')}
			</div>
...
export default CalendarFixedColumn;
