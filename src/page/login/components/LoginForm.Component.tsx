...
export default function LoginForm() {
	...
	const savedUsername = secureLocalStorage.getItem('username');
			...
			login({ username, password })
				...
				.catch((error) => {
					if (error.response) {
						...
						if (responseData) {
							setError(
								`${responseData.error}: ${responseData.messages}`
							);
						} else {
						...
						setError(
							error.message || 'An unidentified error occurred.'
						);
					}
				})
	...
	return (
		<form className='login-form' onSubmit={handleSubmit}>
			<h1 className='login-title'>Coquitlam</h1>
			<div className='mb-4'>
				...
				{username.length === 0 ? (
					<p className='error-label'>
						{ERRORS.employee.username.required}
					</p>
				) : (
					usernameInvalid && (
						<p className='error-label'>
							{ERRORS.employee.username.invalid}
						</p>
					)
				)}
			</div>
			<div className='mb-4'>
				...
				{!password && (
					<p className='error-label'>
						{ERRORS.employee.password.required}
					</p>
				)}
			</div>
